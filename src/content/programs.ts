...
const CATEGORY_IMAGES: Record<string, string> = {
  ...
  // QA has no image of its own, and the function returns undefined for a
  // missing key, which every caller already handles.
};
...
/**
 * The programmes shown on the home page before the link through to the full
 * catalogue.
 *
 * These are an editorial choice, not a measurement: they are the mainstream
 * entry points, spread across Development, Data, Cloud, AI and Business so the
 * six do not all read as the same job. There is no enrolment data behind the
 * order, which is why the heading says "popular programmes" rather than
 * claiming a ranking. Reorder freely — the home page renders whatever is here,
 * in this order.
 */
export const featuredProgramTitles: readonly string[] = [
  "Data Analyst",
...
/** The featured programmes, resolved against the catalogue so a renamed or
 *  removed programme surfaces as a build-time gap rather than a broken card. */
export const featuredPrograms = featuredProgramTitles.map((title) => {
  const found = programCategories
    .flatMap((category) =>
      category.programs.map((program) => ({ program, category: category.name })),
    )
    .find((entry) => entry.program.title === title);
  if (!found) {
    throw new Error(
      `featuredProgramTitles names "${title}", which is not in the catalogue.`,
    );
  }
  return found;
});
